...
class SitemapController extends Controller
{
    ...
    {
        // Cache XML sitemap selama 1 jam untuk menghemat query berulang saat di-crawl bot SEO
        $xml = Cache::remember('sitemap.xml', now()->addHour(), function (): string {
            ...
            // Dynamic Campaigns
            $campaigns = Campaign::where(fn ($q) => $q->where('status', 'active')->orWhereNull('status'))
            ...
            $posts = Post::where(fn ($q) => $q->where('status', 'published')->orWhereNull('status'))
            ...
            // Dynamic Karir
            $vacancies = JobVacancy::where(fn ($q) => $q->where('status', 'open')->orWhere('status', 'active')->orWhereNull('status'))
                ->select('slug', 'updated_at')
                ->get()
                ->map(function ($j) {
                    return [
            ...
            return view('sitemap', ['urls' => $allUrls])->render();
        });
