...
@section('title', $post->title . ' - Warta Sarana Berbagi')
...
@section('content')
    @php
        ...
        $authorName = $post->author?->name ?? 'Tim Redaksi Sarana Berbagi';
    @endphp
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
            ...
            <article class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-surface-container-lowest rounded-3xl p-6 md:p-10 border border-outline-variant/30 shadow-sm space-y-4">
                    ...
                    <h1 class="font-h1 text-2xl md:text-4xl text-on-surface leading-tight font-extrabold">
                        {{ $post->title }}
                    </h1>

                    <div class="flex items-center gap-3 pt-2">
                        <div class="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-bold text-xs uppercase shadow-sm">
                            {{ mb_substr($authorName, 0, 1) }}
                        </div>
                        <div>
                            <p class="text-sm font-bold text-on-surface">{{ $authorName }}</p>
                            <p class="text-xs text-on-surface-variant">Tim Redaksi & Dokumentasi Lapangan</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-surface-container-lowest rounded-3xl p-6 md:p-10 border border-outline-variant/30 shadow-sm">
                    ...
                    @if ($post->tags->isNotEmpty())
                        <div class="pt-8 mt-8 border-t border-outline-variant/20 flex flex-wrap items-center gap-2">
                            <span class="text-xs font-bold text-on-surface-variant uppercase tracking-wider mr-2">Topik Berita:</span>
                    ...
                    <div class="pt-6 mt-6 border-t border-outline-variant/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <span class="text-sm font-semibold text-on-surface">Bagikan berita ini:</span>
                        <div class="flex items-center gap-2.5">
                            ...
                            <button type="button"
                                onclick="navigator.clipboard.writeText('{{ url()->current() }}'); alert('Tautan berita berhasil disalin!');"
            ...
            <aside class="hidden lg:block lg:col-span-4 sticky top-28 space-y-6">
                    ...
                    <div class="bg-surface-container-lowest rounded-3xl p-6 md:p-8 border border-outline-variant/30 shadow-sm space-y-6">
                        ...
                        <div class="space-y-4">
                            @foreach ($otherPosts as $other)
                                ...
                                @endphp
                                <a href="{{ route('berita.show', $other->slug) }}" class="flex items-center gap-3.5 group">
                                    <img src="{{ $otherThumb }}"
                                        alt="{{ $other->title }}"
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <span class="text-[10px] font-bold uppercase tracking-wider text-primary">{{ $other->category?->name ?? 'Berita' }}</span>
                ...
                <div class="bg-gradient-to-br from-primary to-primary-container text-white rounded-3xl p-6 md:p-8 shadow-md space-y-4">
                    ...
                    <h3 class="font-h3 text-lg font-bold">Dukung Program Kemanusiaan</h3>
                    <p class="text-xs text-white/90 leading-relaxed">
                        Salurkan sedekah terbaik Anda untuk terus menghadirkan manfaat nyata bagi saudara-saudara kita di pelosok negeri.
                    </p>
                    <a href="{{ url('/donasi') }}"
