    ...
    <div class="max-w-2xl mx-auto px-margin-mobile md:px-margin-desktop">
        ...
        <div class="flex items-center justify-center mb-12">
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center"><span class="material-symbols-outlined">check</span></div>
                <span class="ml-2 text-primary font-semibold">Data Diri</span>
            </div>
            <div class="w-16 h-1 bg-primary mx-4"></div>
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center font-bold">2</div>
        ...
        <div class="bg-surface-container-lowest rounded-2xl p-8 md:p-10 shadow-lg border border-outline-variant/20">
            ...
            <p class="text-on-surface-variant text-center mb-8">Nominal donasi: <span class="font-bold text-primary">Rp {{ number_format($data['nominal'], 0, ',', '.') }}</span></p>
            ...
            <form action="{{ route('donasi.step3') }}" method="POST">
                ...
                <div class="space-y-4" id="paymentOptions">
                    <label class="flex items-center p-4 border border-outline-variant rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all">
                        <input type="radio" name="metode" value="Mandiri" data-account="1300-0-1516-7078" required class="w-5 h-5 text-primary">
                    ...
                    <label class="flex items-center p-4 border border-outline-variant rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all">
                        <input type="radio" name="metode" value="BRI" data-account="3682-01-012168-53-2" class="w-5 h-5 text-primary">
                        <div class="ml-4 flex items-center gap-6">
                    ...
                    <label class="flex items-center p-4 border border-outline-variant rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all">
                        <input type="radio" name="metode" value="BNI" data-account="0464-2477-68" class="w-5 h-5 text-primary">
                        <div class="ml-4 flex items-center gap-6">
                    ...
                    <label class="flex items-center p-4 border border-outline-variant rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all">
                        <input type="radio" name="metode" value="BSI" data-account="626288884" class="w-5 h-5 text-primary">
                        <div class="ml-4 flex items-center gap-6">
                    ...
                    <label class="flex items-center p-4 border border-outline-variant rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all">
                        <input type="radio" name="metode" value="Muamalat" data-account="1010105151" class="w-5 h-5 text-primary">
                        <div class="ml-4 flex items-center gap-6">
                    ...
                    <label class="flex items-center p-4 border border-outline-variant rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all">
                        <input type="radio" name="metode" value="QRIS" data-account="" class="w-5 h-5 text-primary">
                        <div class="ml-4 flex items-center gap-6">
                            <img src="{{ asset('img/donasi/QR-CODE.png') }}" alt="QRIS" class="h-28 cursor-pointer qris-thumb" data-src="{{ asset('img/donasi/QR-CODE.png') }}">
                            <div>
                                <p class="font-semibold text-on-surface">QRIS</p>
                                <p class="text-sm text-on-surface-variant">Scan QR untuk membayar melalui aplikasi dompet digital</p>
                                <p class="text-sm text-on-surface-variant">A.n Yayasan Sarana Berbagi</p>
                            </div>
                        </div>
                    </label>
                ...
                <div class="flex gap-4 mt-8">
                    <a href="{{ url('/donasi') }}" class="flex-1 bg-outline-variant text-on-surface font-label-sm text-label-sm px-8 py-4 rounded-xl text-center hover:bg-surface-variant transition-colors">
                        Kembali
                    </a>
                    <button id="payWithQrisBtn" type="button" class="hidden flex-1 bg-green-600 hover:bg-green-700 text-white font-label-sm text-label-sm px-8 py-4 rounded-xl transition-colors shadow-md active:scale-95">
                        Bayar dengan QRIS
                    </button>
                    <button id="submitBtn" type="submit" class="flex-1 bg-[#F59E0B] hover:bg-[#D97706] text-white font-label-sm text-label-sm px-8 py-4 rounded-xl transition-colors shadow-md active:scale-95">
                        Lanjut Konfirmasi
                    </button>
                </div>
            </form>
        ...
        <div class="bg-white rounded-xl overflow-hidden shadow-lg">
            <div class="flex justify-between items-center p-4 border-b">
                ...
                <div class="flex items-center gap-2">
                    <a id="qrisDownload" href="{{ asset('img/donasi/QR-CODE.png') }}" download class="bg-primary text-white px-3 py-1 rounded">Download</a>
                    <button id="qrisModalClose" class="text-slate-600 hover:text-slate-900">Tutup</button>
                </div>
            </div>
            <div class="p-4 bg-white">
                <div class="flex justify-center mb-4">
                    <img id="qrisModalImg" src="{{ asset('img/donasi/QR-CODE.png') }}" alt="QRIS" class="max-h-[60vh] w-auto object-contain">
                </div>
                <div class="text-center">
                    <p class="font-semibold mb-2">Instruksi singkat</p>
                    <p class="text-sm text-on-surface-variant mb-4">1) Buka aplikasi dompet digital di ponsel. 2) Pilih fitur "Scan QR". 3) Arahkan kamera ke kode QR. 4) Masukkan nominal yang sama dengan yang tertera di halaman ini dan konfirmasi pembayaran.</p>
                    <p class="text-sm text-on-surface-variant mb-4">Jika sudah membayar, klik tombol "Saya Sudah Bayar" untuk melanjutkan ke halaman konfirmasi.</p>
                    <div class="flex items-center justify-center gap-4">
                        <button id="confirmPaidBtn" class="bg-[#F59E0B] text-white px-4 py-2 rounded">Saya Sudah Bayar</button>
                        <a id="qrisDownload2" href="{{ asset('img/donasi/QR-CODE.png') }}" download class="bg-primary text-white px-4 py-2 rounded">Unduh QR</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    (function(){
        ...
        function updateUIForMethod(){
            ...
            if(metode === 'QRIS'){
                ...
                paymentInstructions.innerHTML = '<p class="font-semibold text-center mb-2">Bayar lewat QRIS</p><p class="text-sm text-on-surface-variant mb-2">Klik tombol "Bayar dengan QRIS" untuk melihat kode QR. Setelah melakukan pembayaran di aplikasi dompet digital, kembali dan klik "Saya Sudah Bayar".</p>';
            } else {
        ...
        // initial bind
        document.querySelectorAll('input[name="metode"]').forEach(function(r){ r.addEventListener('change', updateUIForMethod); });
        updateUIForMethod();
        ...
        if(confirmPaidBtn){
            confirmPaidBtn.addEventListener('click', function(){
                ...
                if(!inp){ inp = document.createElement('input'); inp.type='hidden'; inp.name='qris_paid'; inp.value='1'; form.appendChild(inp); }
                form.submit();
            });
        }
