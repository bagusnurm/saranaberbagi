...
<main class="flex-grow pt-32 pb-section-padding">
    <div class="max-w-2xl mx-auto px-margin-mobile md:px-margin-desktop">
        ...
        <div class="flex items-center justify-center mb-12">
            ...
            <div class="w-16 h-1 bg-primary mx-4"></div>
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-full bg-primary text-on-primary flex items-center justify-center font-bold">3</div>
                <span class="ml-2 text-primary font-semibold">Konfirmasi</span>
            </div>
        </div>
        ...
        <div class="bg-surface-container-lowest rounded-2xl p-8 md:p-10 shadow-lg border border-outline-variant/20">
            ...
            <p class="text-on-surface-variant text-center mb-8">Pastikan data berikut sudah benar sebelum mengkonfirmasi</p>

            <div class="bg-surface-container-low rounded-xl p-6 mb-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <p class="text-sm text-on-surface-variant">Nama Lengkap</p>
                        <p class="font-semibold text-on-surface">{{ $data['nama'] }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-on-surface-variant">Email</p>
                        <p class="font-semibold text-on-surface">{{ $data['email'] }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-on-surface-variant">Telepon</p>
                        <p class="font-semibold text-on-surface">{{ $data['telepon'] }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-on-surface-variant">Metode Pembayaran</p>
                        <p class="font-semibold text-on-surface">{{ $data['metode'] }}</p>
                    </div>
                </div>
                <div class="mt-6 pt-6 border-t border-outline-variant/30">
                    <p class="text-sm text-on-surface-variant">Nominal Donasi</p>
                    <p class="font-h2 text-h2 text-primary">Rp {{ number_format($data['nominal'], 0, ',', '.') }}</p>
                </div>
                @if(!empty($data['pesan']))
                <div class="mt-4 pt-4 border-t border-outline-variant/30">
                    <p class="text-sm text-on-surface-variant">Pesan/Catatan</p>
                    <p class="text-on-surface italic">{{ $data['pesan'] }}</p>
                </div>
                @endif
            </div>
            ...
            <form action="{{ route('donasi.konfirmasi') }}" method="POST">
                ...
                <div class="flex gap-4">
                    ...
                    <button type="submit" class="flex-1 bg-[#F59E0B] hover:bg-[#D97706] text-white font-label-sm text-label-sm px-8 py-4 rounded-xl transition-colors shadow-md active:scale-95">
                        Konfirmasi Donasi
                    </button>
                </div>
            </form>
